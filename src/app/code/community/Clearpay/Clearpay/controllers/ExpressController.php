 ...
 * @author    Clearpay
 * @copyright 2016-2018 Clearpay https://www.clearpay.com
 */

/**
 * Class Clearpay_Clearpay_ExpressController
 *
...
class Clearpay_Clearpay_ExpressController extends Mage_Core_Controller_Front_Action
{
    public function startAction()
    {
        ...
        try {
            ...
            $token = Mage::getModel('clearpay/order')->startExpress($quote);
            $response = array(
                'success' => true,
                'token'  => $token,
            );
    ...
    public function changeAction()
    {
        ...
        $token = $quote->getPayment()->getData('clearpay_token');
    ...
    public function confirmAction()
    {
        ...
        try {
            $order = Mage::getModel('clearpay/order')->getOrderByToken($data['orderToken']);
            ...
            if (!$quote->isVirtual()) {
                ...
                $shipping->setPaymentMethod(Clearpay_Clearpay_Model_Method_Payovertime::CODE);
            } else {
                $billing->setPaymentMethod(Clearpay_Clearpay_Model_Method_Payovertime::CODE);
            }

            $payment = $quote->getPayment();
            $payment->setData('clearpay_token', $data['orderToken']);
            $payment->setMethod(Clearpay_Clearpay_Model_Method_Payovertime::CODE)
                ->save();
            $payment->importData(array(
                'method' => Clearpay_Clearpay_Model_Method_Payovertime::CODE,
            ...
            $quote
                ->setData('clearpay_express_checkout', true)
                ->setData('clearpay_express_amount', json_encode($order['totalAmount']))
                ->setData('clearpay_express_shipping', json_encode($order['shipping']))
                ->save();
            ...
            $helper->log($this->__('Clearpay Payment Gateway Confirmation. QuoteID=%s ReservedOrderID=%s',$quote->getId(), $quote->getReservedOrderId()), Zend_Log::NOTICE);
        ...
        try {
            ...
            $helper->log(
                $this->__(
                    ...
                    $quote->getData('clearpay_order_id'),
                    $quote->getId(),
    ...
    {
        $order = Mage::getModel('clearpay/order')->getOrderByToken($token);
        $order = json_decode(json_encode($order), true);

        if (empty($order['errorCode'])) {
            $quote = Mage::getSingleton('checkout/session')->getQuote();
            $now = Mage::getModel('clearpay/api_adapters_adapterv1')->buildExpressOrderTokenRequest($quote);
