...
class Clearpay_Clearpay_Model_Observer
{
    ...
    /**
     * Update the order limit values for the Clearpay payment methods
     *
    ...
    {
        $configs = array(
            'PBI'                   => 'clearpaypayovertime',
            'PAY_BY_INSTALLMENT'    => 'clearpaypayovertime',
            // 'PAD'                   => 'clearpaybeforeyoupay',
        );
        ...
        foreach ($configs as $tla => $payment) {
            ...
            $base = new Clearpay_Clearpay_Model_Method_Payovertime();
    ...
    public function addLogWarningMessage($observer)
    {
        ...
        // if we are editing the clearpay section of the system configuration
        if (isset ($params['section']) && $params['section'] == 'clearpay') {
            if (Mage::getStoreConfigFlag('clearpay/general/debug') &&
               !Mage::getStoreConfigFlag('dev/log/active')) {
                // Clearpay debug process is enabled, but magento core logging is disabled
                Mage::getSingleton('core/session')->addNotice(
                    Mage::helper('clearpay')->__('Clearpay logging is enabled however core Magento logging is disabled, so Clearpay debug logs will not be written. Please turn on Magento logging at Developer -> Log Settings.')
                );
            }

            if (Mage::helper('clearpay/checkout')->isUsingUnsupportedCheckout()) {
                Mage::getSingleton('core/session')->addNotice(
                    Mage::helper('clearpay')->__('You appear to be using a checkout extension not fully supported by Clearpay. The Clearpay popup will be displayed over a new, empty, page.')
                );
            }
        }
    }
    ...
    /**
     * Set the clearpay order token as part of the order body response
     *
     * @param $observer
     * @return $this
     */
    public function addTokenToOrderResponse($observer)
    {
        ...
        if ($order instanceof Mage_Sales_Model_Order) {
            ...
            if ($payment instanceof Mage_Payment_Model_Info && $payment->getMethodInstance() instanceof Clearpay_Clearpay_Model_Method_Base) {
                $response = Mage::app()->getResponse();
                $helper = Mage::helper('core');
                $responseBody = $helper->jsonDecode($response->getBody());

                $clearpayToken = $payment->getData('clearpay_token');

                $responseBody['clearpayToken'] = $clearpayToken;
                $response->setBody($helper->jsonEncode($responseBody));

                $this->helper()->log('Setting clearpay token to order (' . $order->getIncrementId() . ') response : ' . $clearpayToken, Zend_Log::DEBUG);
            }
        }
        return $this;
    }
