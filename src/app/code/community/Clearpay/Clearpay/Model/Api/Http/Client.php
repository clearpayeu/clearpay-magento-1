<?php
class Clearpay_Clearpay_Model_Api_Http_Client
{
    protected $_url = '';
    ...
    {
        $this->_helper = Mage::helper('clearpay');

        if (!extension_loaded('curl')) {
            throw Mage::exception(
                'Clearpay_Clearpay',
                $this->_helper->__('cURL extension has to be loaded to use this Clearpay Http Client.')
            );
        }
    ...
    public function request($method = 'GET', $data = array())
    {
        ...
        if (!curl_setopt_array($ch, $opts)) {
            ...
            throw Mage::exception('Clearpay_Clearpay', $this->_helper->__('Unable to set options for cURL.'));
        }
